...
def test_tensor_array_write_read():
    ...
    def run(dtype_str, infer_shape, element_shape):
        with tf.Graph().as_default():
            ...
            verify_model([], [], "TensorArrayReadV3:0", mode="vm")
...
def test_tensor_array_scatter():
    ...
    def run(dtype_str, infer_shape):
        with tf.Graph().as_default():
            ...
            _ = tf.get_default_graph()
            verify_model([], [], ["TensorArrayReadV3:0"], mode="vm")
            verify_model([], [], ["TensorArrayReadV3_1:0"], mode="vm")
            verify_model([], [], ["TensorArrayReadV3_2:0"], mode="vm")
            verify_model([], [], ["TensorArrayReadV3_2:0", out4.name], mode="vm")
...
def test_tensor_array_gather():
    ...
    def run(dtype_str, infer_shape):
        with tf.Graph().as_default():
            ...
            _ = tf.get_default_graph()
            verify_model([], [], ["TensorArrayGatherV3:0"], mode="vm")
...
def test_tensor_array_split():
    ...
    def run(dtype_str, infer_shape):
        with tf.Graph().as_default():
            ...
            _ = ta2.read(3)
            _ = tf.get_default_graph()
            verify_model([], [], ["TensorArrayReadV3:0"], mode="debug")
            verify_model([], [], ["TensorArrayReadV3_1:0"], mode="debug")
            verify_model([], [], ["TensorArrayReadV3_2:0"], mode="debug")
            verify_model([], [], ["TensorArrayReadV3_3:0"], mode="debug")
...
def test_tensor_array_concat():
    ...
    def run(dtype_str, infer_shape):
        with tf.Graph().as_default():
            ...
            t = ta2.concat()
            _ = tf.identity(t)
            verify_model([], [], ["Identity:0"], mode="debug")
...
def test_tensor_array_size():
    ...
    def run(dtype_str, infer_shape):
        with tf.Graph().as_default():
            ...
            _ = ta3.size()
            _ = tf.get_default_graph()
            verify_model([], [], "TensorArraySizeV3:0", mode="debug")
...
def test_tensor_array_stack():
    ...
    def run(dtype_str, infer_shape):
        ...
        with tf.Graph().as_default():
            ...
            verify_model([], [], ["TensorArrayStack/TensorArrayGatherV3:0"], mode="vm")
...
def _test_concat_v2(shape1, shape2, dim):
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_data1, np_data2], ["in1:0", "in2:0"], "ConcatV2:0")
...
def _test_sigmoid(data):
    ...
    with tf.Graph().as_default():
        ...
        verify_model(data, "Placeholder:0", "Sigmoid:0")
...
def _test_argx(func, data, **kwargs):
    ...
    with tf.Graph().as_default():
        ...
        verify_model(data, "c0:0", "argx0:0")
...
def _test_variable(data):
    ...
    with tf.Graph().as_default():
        ...
        verify_model(data, "Placeholder:0", "MatMul:0", init_global_variables=True)
    ...
    for adjoint_a in [False, True]:
        for adjoint_b in [False, True]:
            ...
            with tf.Graph().as_default():
                ...
                verify_model([B_np], [B.name], result.name)
...
def _test_sparse_to_dense_v2(indices, values, A_shape, dtype, default_value=None):
    with tf.Graph().as_default():
        ...
        verify_model([], [], result.name, mode="vm")
...
def _test_sparse_add(indices, values, A_shape, B_shape, dtype, flip=False):
    ...
    with tf.Graph().as_default():
        ...
        verify_model([B_np], [B.name], result.name, no_gpu=True)
...
def _test_stridedslice(
    ...
    with tf.Graph().as_default():
        ...
        if len(ip_shape) == 0:  # pylint: disable=len-as-condition
            verify_model(None, "", "strided_slice:0")
        else:
            verify_model(np_data, "in_data:0", "strided_slice:0")
...
def _test_forward_divide(ip_shape, dtype):
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_numer, np_denomin], ["numer:0", "denomin:0"], "RealDiv:0")
...
def _test_forward_floordiv(ip_shape, dtype):
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_numer], ["numer:0"], "FloorDiv:0")
...
def _test_forward_floormod(in_shape, if_shape, dtype):
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_numer, np_factor], ["numer:0", "factor:0"], "FloorMod:0")
...
def _test_forward_truncatemod(ip_shape, dtype):
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_data_1, np_data_2], ["in_data_1:0", "in_data_2:0"], "truncatemod:0")
...
def _test_gather(ip_shape, indice_shape, indice_value, axis, batch_dims, dtype):
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_data, np_indices], ["in_data:0", "indices:0"], out.name)
...
def _test_gather_nd(ip_shape, indice_value, dtype):
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_data], ["in_data:0"], "gather_nd:0")
...
def test_forward_bias_add():
    ...
    def check_bias_add(lh_shpae, rh_shape, dtype):
        ...
        with tf.Graph().as_default():
            ...
            verify_model([lh_data, rh_data], ["lft_data:0", "rgt_data:0"], "BiasAdd:0")
...
def _test_split(in_shape, axis, num_or_size_splits, dtype):
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_data], ["in_data:0"], [n.name for n in relu])
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_data], "in_data:0", concat.name)
...
def _test_forward_top_k_v2(in_shape, k):
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_data], ["in_data:0"], "TopK:0")
...
def _test_unstack(ip_shape, axis, dtype):
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_data], ["in_data:0"], [n.name for n in unstack])
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_data], ["in_data:0"], "stack:0")
...
def _test_tile(in_shape, multiples, dtype):
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_data], ["in_data:0"], "tile:0")
...
def _test_forward_clip_by_value(ip_shape, clip_value_min, clip_value_max, dtype):
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_data], ["in_data:0"], "ClipByValue:0")
...
def _test_resize_bilinear(in_shape, to_shape, align_corners):
    ...
    with tf.Graph().as_default():
        ...
        verify_model(data, "Placeholder:0", "ResizeBilinear:0")
...
def _test_resize_nearest_neighbor(in_shape, to_shape):
    ...
    with tf.Graph().as_default():
        ...
        verify_model(data, "Placeholder:0", "resize_nearest_neighbor:0")
...
def _test_broadcast_args(in_shape_1, in_shape_2):
    ...
    with tf.Graph().as_default():
        ...
        verify_model(None, "", "BroadcastArgs:0", opt_level=0)
...
def _test_broadcast_to(in_shape, to_shape):
    ...
    with tf.Graph().as_default():
        ...
        verify_model(data, "Placeholder:0", "BroadcastTo:0", opt_level=0)
...
def _test_broadcast_to_from_tensor(in_shape):
    ...
    with tf.Graph().as_default():
        ...
        verify_model(data, "Placeholder:0", "BroadcastTo:0")
...
def _test_fill(in_shape):
    ...
    with tf.Graph().as_default():
        ...
        verify_model(in_shape, [], "ones:0", opt_level=1)
...
def _test_fill_from_tensor(in_shape):
    ...
    with tf.Graph().as_default():
        ...
        verify_model(data, "Placeholder:0", "out1:0")
...
def _test_crop(in_shape, off_h, off_w, tar_h, tar_w):
    ...
    with tf.Graph().as_default():
        ...
        verify_model(data, "Placeholder:0", "crop_to_bounding_box/Slice:0")
...
def _test_forward_crop_and_resize(
    ...
    with tf.Graph().as_default():
        ...
        verify_model([image], ["in_data:0"], "crop_and_resize:0", atol=atol, rtol=rtol)
...
def _test_pack(axis, shape, **kwargs):
    ...
    with tf.Graph().as_default():
        ...
        verify_model([a, b], ["pl_a:0", "pl_b:0"], "stack:0")
...
def _test_forward_unpack(in_shape, axis, dtype):
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_data], ["in_data:0"], "Unpack:0")
...
def _test_einsum(equation, dtype, *shape_of_input_tensors):
    ...
    with tf.Graph().as_default():
        ...
        verify_model(input_data, [ph.name for ph in inputs_placeholders], result.name)
...
def test_logical_and():
    with tf.Graph().as_default():
        ...
        verify_model([in_data1, in_data2], ["in1:0", "in2:0"], "out:0")
...
def test_logical_not():
    with tf.Graph().as_default():
        ...
        in_data1 = np.random.choice(a=[False, True], size=(1, 4, 4, 3)).astype("bool")
        verify_model(in_data1, "in1:0", "out:0")
...
def _test_lrn(ishape, size, axis, bias, alpha, beta):
    ...
    with tf.Graph().as_default():
        ...
        verify_model(inp_array, "lrn0_data:0", "lrn:0")
...
def _test_l2_normalize(ishape, eps, axis):
    ...
    with tf.Graph().as_default():
        ...
        verify_model(inp_array, "Placeholder:0", "l2_normalize:0")
...
def _test_forward_transpose(ishape, axes=None):
    ...
    with tf.Graph().as_default():
        ...
        verify_model(data, "transpose_data:0", "transpose:0")
...
def _test_forward_slice_operation_input(input_value, begin_value, size_value):
    ...
    with tf.Graph().as_default():
        ...
        verify_model([input_data], ["input:0"], "slice_output:0")
...
def test_forward_ceil():
    ...
    with tf.Graph().as_default():
        in1 = tf.placeholder(shape=inp_array.shape, dtype=inp_array.dtype)
        tf.ceil(in1)
        verify_model(inp_array, "Placeholder:0", "Ceil:0")
...
def test_forward_floor():
    ...
    with tf.Graph().as_default():
        in1 = tf.placeholder(shape=inp_array.shape, dtype=inp_array.dtype)
        tf.floor(in1)
        verify_model(inp_array, "Placeholder:0", "Floor:0")
...
def test_forward_relu():
    ...
    for mode in ["graph_executor", "vm"]:
        with tf.Graph().as_default():
            in1 = tf.placeholder(shape=inp_array.shape, dtype=inp_array.dtype)
            tf.nn.relu(in1)
            verify_model(inp_array, "Placeholder:0", "Relu:0", mode=mode)
...
def test_forward_leaky_relu():
    ...
    for mode in ["graph_executor", "vm"]:
        with tf.Graph().as_default():
            ...
            verify_model(inp_array, "Placeholder:0", "LeakyRelu:0", mode=mode)
...
def test_forward_elu():
    ...
    with tf.Graph().as_default():
        in1 = tf.placeholder(shape=inp_array.shape, dtype=inp_array.dtype)
        tf.nn.elu(in1)
        verify_model(inp_array, "Placeholder:0", "Elu:0")
...
def test_forward_selu():
    ...
    with tf.Graph().as_default():
        in1 = tf.placeholder(shape=inp_array.shape, dtype=inp_array.dtype)
        tf.nn.selu(in1)
        verify_model(inp_array, "Placeholder:0", "Selu:0")
...
def test_forward_tanh():
    ...
    with tf.Graph().as_default():
        in1 = tf.placeholder(shape=inp_array.shape, dtype=inp_array.dtype)
        tf.nn.tanh(in1)
        verify_model(inp_array, "Placeholder:0", "Tanh:0")
...
def test_forward_softmax():
    ...
    def check_softmax(in_shape, axis, dtype):
        ...
        with tf.Graph().as_default():
            ...
            verify_model([np_data], ["in_data:0"], "Softmax:0")
...
def test_forward_round():
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_data], ["in_data:0"], "round:0")
...
def test_forward_abs():
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_data], ["in_data:0"], "abs:0")
...
def _test_forward_zeros_like(in_shape, dtype):
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_data], ["in_data:0"], "zeros_like:0")
...
def test_forward_squared_difference():
    ...
    with tf.Graph().as_default():
        ...
        verify_model([inp_array_a, inp_array_b], [in1.name, in2.name], out.name)
...
def _test_forward_reverse_v2(in_shape, axis, dtype):
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_data], ["in_data:0"], "reverse:0")
...
def test_forward_sign():
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_data], ["in_data:0"], "sign:0")
...
def test_forward_square():
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_data], ["in_data:0"], "square:0")
...
def test_forward_pow_exp():
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_in1, np_in2], ["in1:0", "in2:0"], "pow:0")
        verify_model([np_in1], ["in1:0"], "exp:0")
...
def test_forward_unary():
    ...
    def _test_forward_unary(op, a_min=1, a_max=5, dtype=np.float32):
        ...
        with tf.Graph().as_default():
            ...
            out = op(in_data)
            verify_model([np_data], ["in_data:0"], out.name)
...
def test_forward_atan2():
    ...
    verify_model([np_data_1, np_data_2], ["in_data_1:0", "in_data_2:0"], "atan2:0")
...
def test_forward_expm1():
    ...
    def _test_forward_expm1(shape):
        ...
        verify_model([np_data], ["in_data:0"], "expm1:0")
...
def test_forward_softsign():
    ...
    def _test_forward_softsign(shape):
        ...
        verify_model([np_data], ["in_data:0"], "softsign:0")
...
def test_forward_rint():
    ...
    def _test_forward_rint(shape):
        ...
        verify_model([np_data], ["in_data:0"], "rint:0")
...
def test_forward_negative():
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_data], ["in_data:0"], "negative:0")
...
def test_forward_log_softmax():
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_data], ["in_data:0"], "LogSoftmax:0")
...
def test_forward_softplus():
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_data], ["in_data:0"], "softplus:0")
...
def test_forward_rsqrt():
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_data], ["in_data:0"], "rsqrt:0")
...
def test_forward_sqrt():
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_data], ["in_data:0"], "sqrt:0")
...
def _test_forward_right_shift(in_shape, dtype):
    ...
    with tf.Graph().as_default():
        ...
        verify_model([lh_data, rh_data], ["lft_data:0", "rgt_data:0"], "RightShift:0")
...
def _test_forward_left_shift(in_shape, dtype):
    ...
    with tf.Graph().as_default():
        ...
        verify_model([lh_data, rh_data], ["lft_data:0", "rgt_data:0"], "LeftShift:0")
...
def test_forward_mean():
    ...
    def check_mean(ishape, **kwargs):
        ...
        with tf.Graph().as_default():
            ...
            verify_model(inp_array, "Placeholder:0", "Mean:0", no_gpu=True)
...
def test_forward_size():
    ...
    def check_size(ishape):
        ...
        with tf.Graph().as_default():
            ...
            verify_model([np_input], ["input:0"], "size:0")
...
def test_forward_reduce():
    ...
    def _check_op(tf_op, ishape, axis, keepdims, dtype="float32"):
        ...
        with tf.Graph().as_default():
            ...
            verify_model([np_data], ["in_data:0"], reduce_op.name)
...
def _test_forward_rel_op(data, func):
    with tf.Graph().as_default():
        ...
        verify_model([data[0], data[1]], ["in1:0", "in2:0"], "out1:0")
...
def _test_forward_expand_dims(data, axis):
    with tf.Graph().as_default():
        ...
        verify_model([data], [in1.name], out.name)
...
def test_forward_maximum():
    ...
    def check_maximum(lh_shape, rh_shape, dtype):
        ...
        with tf.Graph().as_default():
            ...
            verify_model([lh_data, rh_data], ["lft_data:0", "rgt_data:0"], "maximum:0")
...
def test_forward_minimum():
    ...
    def check_minimum(lh_shape, rh_shape, dtype):
        ...
        with tf.Graph().as_default():
            ...
            verify_model([lh_data, rh_data], ["lft_data:0", "rgt_data:0"], "minimum:0")
...
def _test_forward_one_hot(indices_shape, depth, on_value, off_value, axis, out_dtype):
    ...
    with tf.Graph().as_default():
        ...
        verify_model(inp_array1, in1.name, out.name)
...
def _test_forward_add_n(inputs):
    ...
    with tf.Graph().as_default():
        ...
        verify_model(list(inputs), [each.name for each in temp], output.name)
...
def test_sharing_node():
    ...
    with tf.Graph().as_default():
        ...
        verify_model([np_data], ["in_data:0"], "out:0")
...
def _verify_infiniteness_ops(tf_op, name):
    ...
    for tf_dtype in tf_dtypes:
        ...
        verify_model([data], ["in_data:0"], f"{name}:0")
...
def _test_spop_function_invocation_basic():
    with tf.Graph().as_default():
        ...
        verify_model([], [], [t3.name], mode="vm", init_global_variables=True)
...
def _test_spop_function_invocation_defun():
    with tf.Graph().as_default():
        ...
        verify_model([], [], "SpopFnInvocation:0", mode="vm", init_global_variables=True)
...
def _test_spop_arithmetic():
    with tf.Graph().as_default():
        ...
        verify_model([], [], "StatefulPartitionedCall:0", mode="vm", init_global_variables=True)
...
def _test_spop_variables():
    with tf.Graph().as_default():
        ...
        verify_model([], [], "StatefulPartitionedCall:0", init_global_variables=True, mode="vm")
...
def _test_spop_stateful():
    ...
    with tf.Graph().as_default():
        ...
        with pytest.raises(Exception) as execinfo:
            verify_model([], [], [op.name], init_global_variables=True, mode="vm")
...
def _test_unique(n, dtype, is_dyn):
    ...
    with tf.Graph().as_default():
        ...
        tf.unique(in_data)
        if is_dyn:
            verify_model(np_data, "in_data:0", ["Unique:0", "Unique:1"], mode="vm")
        else:
            verify_model(np_data, "", ["Unique:0", "Unique:1"], mode="vm")
...
def test_invert_permutation():
    ...
    with tf.Graph().as_default():
        ...
        verify_model(x, "Placeholder:0", out_name, no_gpu=False)
...
def _test_bincount(in_shape, size, weights):
    with tf.Graph().as_default():
        ...
        verify_model(data, [a.name for a in inputs], result.name, mode="vm")
